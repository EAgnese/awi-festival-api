...
function selectJeux(req, res) {
    ...
    promise.then(
        ...
        res.status(500).send({msg: "Problème sélection des jeux"})
...
function selectJeuById(req, res) {
    ...
    promise.then(
        ...
        res.status(500).send({msg: "Problème sélection du jeu"})
        console.error(error.message)
    })
}
function deleteJeu(req, res) {
    ...
    promise.then(
        ...
        res.status(500).send({msg: "Problème suppression du jeu"})
        console.error(error.message)
    })
}
function createJeu(req, res) {
    ...
    promise.then(
        ...
        res.status(500).send({msg: "Problème création du jeu"})
        console.error(error.message)
    })
}
function updateJeuById(req, res) {
    ...
    promise.then(
        ...
        res.status(500).send({msg: "Problème mise à jour du jeu"})
        console.error(error.message)
    })
}
